...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($check->rowCount() > 0) {
            ...
            // Nettoyage de la table des demandes si succès
            $pdo->prepare("DELETE FROM demandes_acces WHERE email = ?")->execute([$email]);
        }
...
// Récupération des infos URL pour le pré-remplissage
$prefill_nom = $_GET['nom'] ?? '';
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nom" value="<?= htmlspecialchars($prefill_nom) ?>" required placeholder="Ex: Jean Marc" class="input-style w-full">
                        </div>

                        <div>
                            <label class="block text-[10px] uppercase font-bold text-zinc-500 mb-2 ml-2">Email (Login)</label>
                            <input type="email" name="email" value="<?= htmlspecialchars($prefill_email) ?>" required placeholder="staff@prestige.com" class="input-style w-full">
                        </div>
